...
object SudokuMain {
    ...
    var playerSheet = CustomArrayList<CustomArrayList<Int>>()
    ...
    init {
     ...
     private fun CreateSheet() {
        ...
        fun createPlayerSheet() {
            for (row in 0 until 9) {
                var setOfColumns = mutableSetOf<Int>()
                var randomColumnSize = Random.nextInt(3, 7)
                while (setOfColumns.size != randomColumnSize) {
                    setOfColumns.add(Random.nextInt(0, 9))
                }

                for (col in setOfColumns) {
                    playerSheet[row][col] = 0
                }
